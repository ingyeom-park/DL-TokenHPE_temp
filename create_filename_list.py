...
if __name__ == '__main__':
    ...
    os.chdir(args.root_dir) # change directory to current dir './datasets/300W_LP'

    file_counter = 0
    rej_counter = 0
    outfile = open(args.filename, 'w')

    for root, dirs, files in os.walk('.'): # os.walk(): traverse all sub-dirs
        for f in tqdm(files):
            if f[-4:] == '.jpg': 
                mat_path = os.path.join(root, f.replace('.jpg', '.mat')) # f=f.replace(old,new)
